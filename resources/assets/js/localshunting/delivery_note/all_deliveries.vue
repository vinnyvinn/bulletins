...
<script>
    export default {
        created() {
            ...
            http.get('/api/ls-delivery/?s=' + window.Laravel.station_id).then(response => {
        ...
        methods: {
            ...
            edit(delivery) {
                window._router.push({ path: '/ls/delivery/' + delivery.id + '/edit' })
            },

            unload(delivery) {
                window._router.push({ path: '/ls/delivery/' + delivery.id + '/unload' })
            },

            destroy(id) {
                this.$root.isLoading = true;

                http.destroy('api/ls-delivery/' + id + '/?s=' + window.Laravel.station_id).then(response => {
