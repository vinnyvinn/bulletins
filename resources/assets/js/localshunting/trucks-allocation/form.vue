<template>
  <div class="container">
    ...
    <div class="row" v-show="showTrucks">
      <div class="col-md-6">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="table-responsive">
              <table class="table no-wrap dataTable">
                <thead>
                  <tr>
                    <th>Plate #</th>
                    <th>Trailer</th>
                ...
                <tbody>
                  <tr v-for="truck in trucks">
                    ...
                    <td>
                      <span v-if="truck.trailer">{{ truck.trailer.plate_number }}</span>
                    </td>
                    <td>
                      <span v-if="truck.driver">{{ truck.driver.first_name }}</span>
                    </td>
                    <td>
                      <button v-if="$root.can('ls-create-allocation') || $root.can('ls-edit-allocation')" type="button" @click="allocate(truck)" name="button" class="btn btn-sm btn-success">Add</button>
                    </td>
                  </tr>
                </tbody>
              ...
              <table class="table no-wrap">
                ...
                <tbody>
                  <tr v-for="allocatedtruck in allocation.allocatedtrucks">
                    ...
                    <td>
                      <span v-if="allocatedtruck.trailer"> {{ allocatedtruck.trailer.plate_number}} </span>
                    </td>
                    <td>
                      <span v-if="allocatedtruck.driver"> {{ allocatedtruck.driver.first_name }} </span>
                    </td>
                    <td>
                      <button v-if="$root.can('ls-delete-allocation') || $root.can('ls-edit-allocation')" type="button" @click="toggleRemoveTruck(allocatedtruck)" class="btn btn-sm btn-danger">
                        <i class="fa fa-minus"></i>
                      </button>
                    </td>
                  </tr>
                  <tr v-show="unallocateTruck">
            ...
            <table class="table no-wrap dataTable">
              ...
              <tbody>
                <tr v-for="employee in employees" v-if="employee.category == 'supervisor'">
                  ...
                  <td>
                    <button type="button" @click="allocateEmployee(employee)" name="button" class="btn btn-sm btn-success">Add</button>
                  </td>
                  <td></td>
                </tr>
              </tbody>
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Unallocate Truck (
              <strong>{{ activeTruck.plate_number}} </strong>)</h4>
          </div>
          ...
          <div class="modal-footer">
            ...
            <button type="button" @click="unallocate()" name="button" class="btn btn-sm btn-danger">Remove Truck</button>
            </td>
          </div>
        </div>
  ...
  export default {
    ...
    created() {
      ...
      http.get('/api/journey/create').then(response => {
        ...
        .then(() => {
          return http.get('/api/unallocated_employees').then(response => {
        ...
        .then(() => {
          return http.get('/api/contract-trucks/' + this.$route.params.id).then(response => {
