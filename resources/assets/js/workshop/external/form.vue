                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <select required v-model="requisition.job_card_id" name="job_card_id" id="job_card_id" class="form-control input-sm">
                            ...
                            <div class="form-group">
                                ...
                                <select required v-model="requisition.vendor_id" name="vendor_id" id="vendor_id" class="form-control input-sm">
                                    <option v-for="vendor in vendors" :value="vendor.DCLink">{{ vendor.Name }} ({{ vendor.Account }})</option>
                                </select>
                            </div>
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <select required v-model="requisition.type" name="type" id="type" class="form-control input-sm">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" class="form-control" v-model="requisition.approximate_cost" id="approximate_cost">
                            </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <textarea v-model="requisition.mechanic_findings" name="mechanic_findings" id="mechanic_findings" cols="20" rows="5" class="form-control input-sm">{{ card.mechanic_findings }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row" v-if="isParts">
                        <div class="col-sm-7">
                            <div class="form-group">
                                ...
                                <select name="item_id" v-model="item.item_id" id="item_id" class="form-control input-sm">
                                    <option value="null" disabled selected>Select Part</option>
                                    <option v-for="item in filteredParts" :value="item.StockLink">{{ item.Description_1 }} ({{ item.product_make }} {{ item.product_model }})</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <input number type="number" min="1" class="form-control"
                                       onfocus="this.select()"
                                       name="requested_quantity" id="requested_quantity" v-model="item.quantity">
                            </div>
                        </div>
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                <label>Actions</label>
                                <a @click="addToList" class="btn btn-success btn-block">Add to Service</a>
                            </div>
                        </div>
...
<script>
    export default {
      ...
      computed: {
        ...
        filteredParts() {
          return this.parts.filter(part => this.partMatches(part));
        }
      },
      ...
      methods: {
        partMatches(part) {
          return (
            (part.product_make
              .toLowerCase()
              .indexOf(this.card.vehicle.make.name.toLowerCase()) !== -1 ||
              part.product_make.toLowerCase().indexOf("all vehicles") !== -1) &&
            (part.product_model
              .toLowerCase()
              .indexOf(this.card.vehicle.model.name.toLowerCase()) !== -1 ||
              part.product_model.toLowerCase().indexOf("all") !== -1)
          );
        },
        back() {
          window._router.go(-1);
