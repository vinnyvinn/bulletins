...
<script>
    export default {
        created() {
            http.get('/api/progress/loading').then(response => {
        ...
        methods: {
            progresssss(truck) {
                http.post('/api/progress/loading/' + truck.id + '/edit', {}).then(response => {
                    alert2(this.$root, [response.message], 'success');
                    this.trucks = response.trucks;
                });
            },
            progress(truck) {
                window._router.push({path: '/progress/loading/' + truck.id});
            },
        }
    }
</script>
