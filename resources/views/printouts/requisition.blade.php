<div class="container" style="font-size: 1.1em; margin-top: -40px; background: #fff">
    <div class="row">
        ...
        <div class="col-xs-4 text-right">
            ...
            <h3 class="text-right"><strong style="color: red !important;">REQ
                    - {{ str_pad($requisition->id, 5, '0', STR_PAD_LEFT) }}</strong></h3>
        </div>
    </div>
    ...
    <div class="row" style="border: 1px solid #e5e5e5;">
        ...
        <div class="col-xs-4"><h4 class="text-right" style="text-align: left">JC-{{ $requisition->job_card_id }}</h4>
        </div>
        <div class="col-xs-2"><h4 class="text-left" style="text-align: right"><strong>Date</strong></h4></div>
        <div class="col-xs-4"><h4
                    class="text-right">{{ Carbon\Carbon::parse($requisition->time_in)->format('dS F Y, h:i A') }}</h4>
        </div>

        <div class="col-xs-2"><h4 class="text-left"><strong>KM</strong></h4></div>
        <div class="col-xs-4"><h4 class="text-right"
                                  style="text-align: left">{{ number_format($requisition->jobCard->current_km_reading, 0) }}</h4>
        </div>
    ...
    <div class="row">
        ...
        @if($requisition->status == \SmoDav\Support\Constants::STATUS_ISSUED)
            ...
            <div class="row">
                ...
                <div class="col-xs-4"><h6 class="text-left"
                                          style="text-align: left"> {{$requisition->issuedby_user}}</h6></div>
        ...
        @else
            <div class="row">
                ...
                <div class="col-xs-4"><h6 class="text-left"
                                          style="border-bottom: 1px solid #e5e5e5">{{$requisition->approved_by_user}}
                        &nbsp;</h6></div>
        ...
        <div class="col-xs-12">
            Unless authorized signature with proper date & time appears on this form no goods will be released from the
            stores.
        </div>
    </div>
